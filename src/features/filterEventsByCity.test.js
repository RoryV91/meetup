	...
	test("User can select a city from the suggested list.", ({
		...
		and(
			...
			async () => {
                ...
                // Only compare the first 32 events (max of 32 events are returned by default, even if there are more events in the list)
                expect(EventListItems).toHaveLength(Math.min(32, berlinEvents.length));
			}
		);
	});
});
